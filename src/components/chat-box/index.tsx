...
export const ChatBox: FC<ChatBoxProps> = () => {
    ...
    return (
        <div
            className={cn(
                "cb-flex cb-flex-col cb-bg-muted-background cb-bg-opacity-90 cb-backdrop-blur-lg cb-backdrop-filter cb-transition-all",
                {
                    "cb-fixed cb-inset-0 cb-z-[1000] sm:cb-bottom-6 sm:cb-left-[unset] sm:cb-right-6 sm:cb-top-[unset] sm:cb-h-[35rem] sm:cb-w-[25rem] sm:cb-rounded-xl":
                        !options.renderAsBlock,
                    "cb-h-full cb-w-full": options.renderAsBlock,
        ...
        >
            <div
                className={cn("cb-flex cb-flex-1 cb-flex-col cb-overflow-hidden", {
                    "cb-border-x-2": options.renderAsBlock,
                })}
            >
                <ChatBoxHeader />
                <ChatBoxContent />
